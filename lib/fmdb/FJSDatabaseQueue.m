...
@interface FJSDatabaseQueue () {
    dispatch_queue_t    _queue;
    FJSDatabase          *_db;
}
@end

@implementation FJSDatabaseQueue

+ (instancetype)databaseQueueWithPath:(NSString *)aPath {
    FJSDatabaseQueue *q = [[self alloc] initWithPath:aPath];
...
+ (instancetype)databaseQueueWithPath:(NSString *)aPath flags:(int)openFlags {
    FJSDatabaseQueue *q = [[self alloc] initWithPath:aPath flags:openFlags];
...
- (void)inDatabase:(__attribute__((noescape)) void (^)(FJSDatabase *db))block {
#ifndef NDEBUG
    /* Get the currently executing queue (which should probably be nil, but in theory could be another DB queue
     * and then check it against self to make sure we're not about to deadlock. */
    FJSDatabaseQueue *currentSyncQueue = (__bridge id)dispatch_get_specific(kDispatchQueueSpecificKey);
...
- (void)beginTransaction:(FMDBTransaction)transaction withBlock:(void (^)(FJSDatabase *db, BOOL *rollback))block {
    FMDBRetain(self);
    dispatch_sync(_queue, ^() { 
...
- (void)inTransaction:(__attribute__((noescape)) void (^)(FJSDatabase *db, BOOL *rollback))block {
    [self beginTransaction:FMDBTransactionExclusive withBlock:block];
}

- (void)inDeferredTransaction:(__attribute__((noescape)) void (^)(FJSDatabase *db, BOOL *rollback))block {
    [self beginTransaction:FMDBTransactionDeferred withBlock:block];
}

- (void)inExclusiveTransaction:(__attribute__((noescape)) void (^)(FJSDatabase *db, BOOL *rollback))block {
    [self beginTransaction:FMDBTransactionExclusive withBlock:block];
}

- (void)inImmediateTransaction:(__attribute__((noescape)) void (^)(FJSDatabase * _Nonnull, BOOL * _Nonnull))block {
    [self beginTransaction:FMDBTransactionImmediate withBlock:block];
}

- (NSError*)inSavePoint:(__attribute__((noescape)) void (^)(FJSDatabase *db, BOOL *rollback))block {
